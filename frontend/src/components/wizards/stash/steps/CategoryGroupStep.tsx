/**
 * CategoryGroupStep - Category group selection for stash wizard
 *
 * Allows user to select a default category group for new stash items.
 */

import { SearchableSelect } from '../../../SearchableSelect';
import type { CategoryGroup } from '../../../../types';

interface CategoryGroupStepProps {
...
export function CategoryGroupStep({
  groups,
  selectedGroupId,
  onSelectGroup,
  loading,
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="mb-6" style={{ color: 'var(--monarch-text-muted)' }}>
        Select where your stash savings categories will be created in Monarch Money.
      </p>
      ...
      {!loading && groups.length > 0 && (
        <div className="mb-4">
          ...
          <SearchableSelect
            ...
            aria-labelledby="stash-category-group"
          />
          <p className="mt-2 text-sm" style={{ color: 'var(--monarch-text-muted)' }}>
            Each stash will create a dedicated category in this group.
          </p>
        </div>
      )}
