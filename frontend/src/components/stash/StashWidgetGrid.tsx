...
function createLayoutFromItems(items: StashItem[], cols: number): LayoutItem[] {
  ...
  const handleDragOrResizeStop = useCallback(
    (layout: Layout) => {
      ...
      layoutChangeTimeoutRef.current = setTimeout(() => {
        const updates: StashLayoutUpdate[] = lgLayout.map((l) => ({
          id: l.i,
          grid_x: l.x,
          grid_y: l.y,
          col_span: l.w,
      ...
      >
        {items.map((item, index) => (
          <div
            ...
            data-tour={index === 0 ? 'stash-move-card' : undefined}
          >
            <StashCard
              item={item}
              onEdit={onEdit}
