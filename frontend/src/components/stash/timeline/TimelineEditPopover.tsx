...
import { motion, AnimatePresence, scaleVariants } from '../../motion';
...
export function TimelineEditPopover({
  ...
  // Reset form state when switching between create/edit or when initialDate changes
  // Using useLayoutEffect to reset synchronously before paint
  const resetKey = `${event?.id ?? 'new'}-${initialDate}`;
  const prevResetKeyRef = useRef(resetKey);
  useLayoutEffect(() => {
    if (prevResetKeyRef.current !== resetKey) {
      prevResetKeyRef.current = resetKey;
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Form reset on prop change is a valid pattern
      setName(event?.name ?? '');
      setType(event?.type ?? 'deposit');
      setItemId(event?.itemId ?? itemConfigs[0]?.itemId ?? '');
      setAmount(event?.amount?.toString() ?? '');
      setDate(event?.date ?? initialDate);
      setError(null);
    }
  }, [resetKey, event, initialDate, itemConfigs]);
