...
function getStackRotation(position: number, ideaId: string): number {
  ...
  let positionAngle = -2; // default for position 2 (top)
  if (position === 0) {
    positionAngle = -8;
  } else if (position === 1) {
    positionAngle = 6;
...
// eslint-disable-next-line sonarjs/cognitive-complexity -- Animation component handles multiple visual states (stacking, morphing, dev cycle stages)
export function IdeaBubble({
  ...
  const getBorderStyle = () => {
    ...
    if (isInProgress || isMorphingToDev)
      return { borderColor: 'var(--monarch-info)', borderWidth: '2px' };
    return {};
  };
  ...
  const getStageBadge = () => {
    ...
    if (isInProgress || isMorphingToDev)
      return { icon: Code, label: 'In Progress', color: 'var(--monarch-info)' };
    return null;
  };
  ...
  return (
    <div
      className={`rounded-xl border bg-(--monarch-bg-elevated) p-4 ${shadowClass} ${animationClass} ${stackedOnClass} ${isStacked && !isInDevCycle ? 'absolute left-0 right-0 top-0' : 'relative'} transition-all duration-300`}
      style={{
    ...
    >
      ...
      {showDevFeatures && (
        <span
          className={`absolute -top-2 right-3 px-2 py-0.5 rounded text-[10px] font-medium bg-(--monarch-bg-page) text-(--monarch-text-muted) border border-(--monarch-border) ${isMorphingToDev ? 'morph-element-in' : ''}`}
        >
          Demo
        </span>
      )}
      ...
      {/* Idea content */}
      <h3 className="text-sm font-medium text-(--monarch-text-dark) leading-snug mb-3 line-clamp-2">
        {idea.title}
      </h3>
      ...
      {!isInDevCycle && (
        <div className={`flex items-center gap-2 ${isMorphingToDev ? 'morph-element-out' : ''}`}>
          <div
            className={`flex items-center gap-1.5 px-2 py-1 rounded-full bg-(--monarch-bg-page) ${
              showVoteAnimation && !reducedMotion ? 'upvote-bounce' : ''
            }`}
          >
            <ThumbsUp
              className={`h-3.5 w-3.5 transition-colors ${
                showVoteAnimation ? 'text-(--monarch-orange)' : 'text-(--monarch-text-muted)'
              }`}
            />
            <span
              className={`text-xs font-semibold ${
                showVoteAnimation && !reducedMotion ? 'vote-count-roll' : ''
              } ${showVoteAnimation ? 'text-(--monarch-orange)' : 'text-(--monarch-text-dark)'}`}
            >
              {displayedVotes}
            </span>
          </div>
          ...
          {/* Category badge */}
          <span className="text-xs px-2 py-0.5 rounded-full bg-(--monarch-orange)/10 text-(--monarch-orange) font-medium">
            {idea.category}
          </span>
        </div>
      )}
      ...
      {isShipped && (
        <div className="flex items-center gap-2 mt-3">
          ...
          <span className="text-sm font-medium" style={{ color: 'var(--monarch-success)' }}>
            Feature delivered!
          </span>
        </div>
      )}
