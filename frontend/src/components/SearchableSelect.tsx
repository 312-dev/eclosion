  ...
  const renderOption = (opt: SelectOption) => {
    ...
    return (
      ...
      >
        ...
        {isSelected && (
          <Check size={16} className="text-(--monarch-orange) shrink-0" />
        )}
      </button>
    );
  };
      ...
      {dropdown.isOpen && (
        <Portal>
          ...
          <div
            ...
            className="fixed z-50 min-w-50 max-w-75 rounded-lg shadow-lg border border-(--monarch-border) bg-(--monarch-bg-card) overflow-hidden dropdown-menu"
            style={{
            ...
            <div className="max-h-60 overflow-y-auto">
              {loading ? (
                ...
                filteredGroups?.map((group, idx) => (
                  <div key={group.label}>
                    {idx > 0 && (
                      <div className="h-px bg-(--monarch-border) mx-2" />
                    )}
                    <div className="px-3 py-1.5 text-xs font-medium uppercase tracking-wide text-(--monarch-text-muted) bg-(--monarch-bg-page)">
                      {group.label}
                    </div>
