...
export function useUpdateSettingsMutation() {
  ...
  return useMutation({
    ...
    }) => (isDemo ? demoApi.updateSettings(settings) : api.updateSettings(settings)),
    onSuccess: () => {
...
export function useSetConfigMutation() {
  ...
  return useMutation({
    mutationFn: ({ groupId, groupName }: { groupId: string; groupName: string }) =>
      isDemo ? demoApi.setConfig(groupId, groupName) : api.setConfig(groupId, groupName),
    onSuccess: () => {
...
export function useImportSettingsMutation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      data: Parameters<typeof api.importSettings>[0];
      options?: Parameters<typeof api.importSettings>[1];
    }) => (isDemo ? demoApi.importSettings(data, options) : api.importSettings(data, options)),
    onSuccess: () => {
...
/**
 * Update acknowledgement states (tours, intros, update read status)
 */
export function useUpdateAcknowledgementsMutation() {
  const isDemo = useDemo();
  const smartInvalidate = useSmartInvalidate();
  return useMutation({
    mutationFn: (data: Partial<AcknowledgementsData>) =>
      isDemo ? demoApi.updateAcknowledgements(data) : api.updateAcknowledgements(data),
    onSuccess: () => {
      smartInvalidate('updateAcknowledgements');
    },
  });
}
