 ...
 * Stash Image Storage
 *
 * Handles saving and deleting custom images for stash items.
 * Images are stored in the app data directory under 'stash-images/'.
 */
...
/** Directory name for stash images */
const IMAGES_DIR = 'stash-images';

/**
 * Get the stash images directory path.
...
export function getImagesDir(): string {
  ...
  if (!fs.existsSync(dir)) {
    ...
    debugLog(`Created stash images directory: ${dir}`);
  }
  return dir;
}
...
/**
 * Save a stash item image from base64 data.
 *
 * @param itemId - The stash item ID (used as filename base)
 ...
 */
export function saveStashImage(itemId: string, base64Data: string): SaveImageResult {
  try {
    ...
    debugLog(`Saved stash image: ${filePath}`);
    ...
    debugLog(`Failed to save stash image: ${message}`);
...
function deleteExistingImages(safeId: string): void {
  ...
  for (const ext of extensions) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      debugLog(`Deleted old stash image: ${filePath}`);
    }
  }
}
...
export function deleteStashImage(imagePath: string): boolean {
  try {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      debugLog(`Deleted stash image: ${fullPath}`);
    }

    return true;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    debugLog(`Failed to delete stash image: ${message}`);
    return false;
  }
}
