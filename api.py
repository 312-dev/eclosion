...
def _enforce_desktop_secret():
    ...
    Returns error response or None if validation passes.
    """
    ...
    if not provided_secret or not secrets.compare_digest(provided_secret, config.DESKTOP_SECRET):
        _audit_log("DESKTOP_AUTH", False, "Invalid or missing desktop secret")
...
def security_checks():
    ...
    if error_response := _enforce_desktop_secret():
